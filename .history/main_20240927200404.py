...
# Survole par son XPATH de toutes les catégories dans la barre de navigation
menu = driver.find_element(By.XPATH, '//*[@id="the-new-header"]/div[2]/div/div[1]/div[1]/div[1]')
action.move_to_element(menu).perform()
time.sleep(3)

# Survole de <Mes catégories> dans le menu par son XPATH
...
for categorie in list_categories:
    ...
    print(f"- Categorie :{categorie.text}")
    #temporiaer pour que notre scrappe se fasse corectement 
    time.sleep(2)
    list_sous_categorie = driver.find_elements(By.CSS_SELECTOR, "div.final-cate.has-more span")
    for sous_categorie in list_sous_categorie:
        print(f"\t - Sous Categorie{sous_categorie.text}")
