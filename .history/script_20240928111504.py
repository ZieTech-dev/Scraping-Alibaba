    ...
    def ScrapProduit(self):
        try:
            ...
            for produit in list_produits:
                try:
                    # Sélectionner les images à l'intérieur des liens de la carte produit
                    imgs_src = produit.find_elements(By.CSS_SELECTOR, "a.search-card-e-slider__link.search-card-e-slider__gallery img")
                    for img in imgs_src:
                        img_src = img.get_attribute("src")
                        if img_src:
                            print(f"Image URL: {img_src}")
                except Exception as e:
                    print(f"Erreur lors de la récupération de l'image pour le produit : {e}")
        except Exception as e:
